...
description: Real examples from Supabase, Vercel, and a game admin portal on where dashboards still beat CLI and MCP
...
The question I ask is: does this task require sequential steps, state across multiple apps, or judgment that's hard to verify? If yes, CLI or MCP. If it's a single lookup or toggle, the dashboard. And when the task needs judgment an AI agent can only guess at, like re-categorizing 200 rows correctly, I'd rather do it myself than risk it getting some wrong at scale. If it were something a script could determine reliably, I'd automate it.

That's what settled my worry about the dashboard becoming irrelevant. CLI and MCP can technically do everything the dashboard does, but capability was never the question. The dashboard wins on convenience often enough to stay worth keeping around.
